        ...
        <form action="{{ route('admin.email-templates.update', $emailTemplate->key) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="mt-6">
                        <label for="available_variables_json" class="block text-sm font-medium text-gray-700 mb-2">Variables JSON (optional)</label>
                        @php
                            $vars = is_array($emailTemplate->available_variables) ? $emailTemplate->available_variables : [];
                            $assoc = [];
                            foreach ($vars as $v) { $assoc[$v] = $assoc[$v] ?? ''; }
                            $prefillJson = old('available_variables_json', json_encode($assoc, JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES));
                        @endphp
                        <textarea id="available_variables_json" name="available_variables_json" rows="10"
                                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                  placeholder='{"client_name": "Client\'s name"}'>{!! $prefillJson !!}</textarea>
                        <p class="text-xs text-gray-500 mt-2">
                            Tip: paste a JSON object like {"unit_name":"Unit name","client_name":"Client's name"}.
                            If provided, this JSON will be used to derive variable names (its keys) and will override the inputs above.
                        </p>
                    </div>
