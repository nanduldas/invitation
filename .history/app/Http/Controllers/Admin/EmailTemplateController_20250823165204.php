...
class EmailTemplateController extends Controller
{
    ...
    private function normalizeAvailableVariables(Request $request): array
    {
        ...
        // Accept multiple possible input names for the textarea field
        $raw = $request->input('available_variables_json');
        $hasJson = is_string($raw) && trim($raw) !== '';

        if (!$hasJson) {
            $raw = $request->input('variables_json'); // alternate name sometimes used by UIs
            $hasJson = is_string($raw) && trim($raw) !== '';
        }
    ...
    public function store(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            // available_variables is normalized manually; no validation rule needed
